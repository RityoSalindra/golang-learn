...
// teknik penulisan query di awal dengan kelebihan bisa di re-use atau digunakan banyak kali untuk eksekusi yang berbeda-beda.
func sqlPrepare() {
	db, err := connect()
	if err != nil {
	...
	stmt, err := db.Prepare("select name, grade from tb_student where id = ?") //digunakan untuk deklarasi query, yang mengembalikan objek bertipe sql.*Stmt
	if err != nil {
		fmt.Println(err.Error())
		return
	}
