...
// Salt yang digunakan adalah hasil dari ekspresi time.Now().UnixNano().
// Hasilnya akan selalu unik setiap detiknya, karena scope terendah waktu pada fungsi tersebut adalah nano second atau nano detik.
func main() {
	var text = "this is secret"
	fmt.Printf("original : %s\n\n", text)

	var hashed1, salt1 = doHashUsingSalt(text) //Hasil ekripsi fungsi doHashUsingSalt akan selalu beda, karena salt yang digunakan adalah waktu.
